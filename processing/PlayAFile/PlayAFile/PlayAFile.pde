import ddf.minim.*;
Minim minim;
AudioPlayer player;

void setup()
{
  size(1000, 1000, P3D);

  minim = new Minim(this);

  player = minim.loadFile("stutter.mp3");
}

void draw()
{
  background(0);
  stroke(255);

  for(int i = 0; i < player.bufferSize() - 1; i++)
  {
    float x1 = map( i, 0, player.bufferSize(), 0, width );
    float x2 = map( i+1, 0, player.bufferSize(), 0, width );
    line( x1, 50 + player.left.get(i)*50, x2, 50 + player.left.get(i+1)*50 );
    line( x1, 150 + player.right.get(i)*50, x2, 150 + player.right.get(i+1)*50 );
  
    stroke(255,0,0);
    beginShape();
    curveVertex(0, (sin(player.right.get(i)))*50+100);
    curveVertex(x1, (player.right.get(i))*50+100);
    curveVertex(x2, (player.right.get(i))*50+100);
    curveVertex(1000, (player.right.get(i))*50+ 100);
    endShape();
    
    line(0, player.right.get(i)*40+250, 1000, player.right.get(i)*40+250);
    
    triangle(500, sin(player.right.get(i))*40+450, 500,sin(player.right.get(i))*40+450, 450, cos(player.right.get(i))*40+450);
  }
  

}

void keyPressed()
{
  if ( player.isPlaying() )
  {
    player.pause();
  }
  else if ( player.position() == player.length() )
  {
    player.rewind();
    player.play();
  }
  else
  {
    player.play();
  }
}
